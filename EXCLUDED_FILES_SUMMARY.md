...
| frozen model joblib and split-ID payloads | outside the version 1.0.0 redistribution scope; some files are large or dataset-derived | hashes/metadata are included; regenerate locally or use an exact local copy verified against the inventory | yes |
...
| old release-manifest utility | hardcoded obsolete inclusion policy (including internal checklist) | use the version 1.0.0 public manifest and validator | yes |
| `.zenodo.json` | not required by version 1.0.0; permanent citation metadata is in `CITATION.cff` | outside the version 1.0.0 file set | not applicable |
...
Version 1.0.0 contains no file larger than GitHub's ordinary 100 MB
